...
function aplicarSpread() {
  ...
  // CAPTURA REAL DE TUS INPUTS
  const nivel     = parseInt(document.getElementById('spread-nivel').value);
  const bonusAtk  = parseInt(document.getElementById('spread-bonus').value);
  const habilidad = document.getElementById('spread-habilidad').value.trim();

  // Validación simple: si el usuario no pone nada, avisamos
  if (isNaN(nivel) || isNaN(bonusAtk)) {
    mostrarOutput('output-spread', 'error', '⚠ Por favor, rellena el Nivel y el Bonus de tu sección.');
    return;
  }
  ...
  const personajeMejorado = {
    ...personaje,
    nivel:      nivel,
    ataque:     personaje.ataque + bonusAtk,
    habilidad:  habilidad || "Sin habilidad"
  };

  mostrarOutput('output-spread', 'success',
    `<strong>¡Objeto clonado con éxito!</strong><br><br>` +
    `Nuevo Nivel: ${personajeMejorado.nivel}<br>` +
    `Ataque Total: ${personajeMejorado.ataque} (Base ${personaje.ataque} + Bonus ${bonusAtk})`
  );
}
