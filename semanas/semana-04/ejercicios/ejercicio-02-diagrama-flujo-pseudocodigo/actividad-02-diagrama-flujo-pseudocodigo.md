...
.
Solo si cumple ambas condiciones obligatoriamente, mostrar "Puede pasar"
.
Si no cumple alguna de las dos, mostrar "Acceso denegado"
.
2. El Pseudocódigo (La estructuración)
: (Utilizamos el operador lógico "Y" para evaluar ambas condiciones de forma elegante).
INICIO
    LEER edad
...
.
SI es menor a 30%: Abrir aspersores durante 5 minutos y volver al inicio para medir de nuevo (repetir)
.
Si ya es mayor o igual a 30%, el riego termina.
2. El Pseudocódigo (La estructuración)
: (Aquí usamos la estructura cíclica MIENTRAS para que dé vueltas)
INICIO
    LEER humedad
    
    MIENTRAS humedad < 30 HACER
        ESCRIBIR "Abriendo aspersores durante 5 minutos"
        // Vital: Volver a leer la humedad para evitar un bucle infinito
        LEER humedad 
    FIN MIENTRAS
